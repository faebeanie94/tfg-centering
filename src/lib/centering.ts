...
/** @deprecated Prefer card format presets in card-sizes.ts */
...
export interface CenteringResult {
  ...
  /** Isotropic pixels per millimetre from the selected card format. */
  pxPerMm: number;
}
 ...
 * as the selected format’s manufactured size. One isotropic px/mm keeps L/R/T/B
 * consistent even if the outer box is slightly off-aspect.
 */
